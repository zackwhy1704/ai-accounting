                            ...
                            <TableRow key={bill.id} className="border-border hover:bg-muted/50">
                              ...
                              <TableCell className="text-right">
                                <RowActionsMenu actions={[
                                  ...
                                  { label: "3-Way Match", icon: <PackageCheck className="h-3.5 w-3.5" />, onClick: () => navigate(`/purchases/bills/${bill.id}/match`) },
                                  ...
                                  { label: "Void Bill", icon: <XCircle className="h-3.5 w-3.5" />, onClick: () => { if ((bill.amount_paid ?? 0) > 0) { toast("This bill has payments applied. Please void the payments first before voiding the bill.", "warning"); return } if (confirm("Void this bill? This will reverse any GL entries.")) updateBillStatus.mutate({ id: bill.id, status: "void" }, { onSuccess: () => toast("Bill voided", "success"), onError: (e: any) => toast(e?.response?.data?.detail ?? "Failed to void bill", "warning") }) }, danger: true, dividerBefore: true, disabled: isVoid },
                                  { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if ((bill.amount_paid ?? 0) > 0) { toast("This bill has payments applied. Please void the payments first before deleting.", "warning"); return } if (!isVoid && !isDraft) { toast("Please void this bill first before deleting.", "warning"); return } if (confirm(`Delete bill ${bill.bill_number}? This cannot be undone.`)) deleteBill.mutate(bill.id, { onSuccess: () => toast("Bill deleted", "success"), onError: (e: any) => toast(e?.response?.data?.detail ?? "Failed to delete bill", "warning") }) }, danger: true },
                                ]} />
                              </TableCell>
                            </TableRow>
                          )
