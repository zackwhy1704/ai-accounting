                      ...
                      <TableRow key={grn.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if (grn.status === "billed") { toast("This GRN has been billed and cannot be deleted.", "warning"); return } if (confirm(`Delete GRN ${grn.grn_number}? This cannot be undone.`)) api.delete(`/goods-received-notes/${grn.id}`).then(() => { queryClient.invalidateQueries({ queryKey: ["goods-received-notes"] }); toast("GRN deleted", "success") }).catch((e: any) => toast(e?.response?.data?.detail ?? "Failed to delete GRN", "warning")) }, danger: true, dividerBefore: true, disabled: grn.status === "billed" },
                          ]} />
                        </TableCell>
                      </TableRow>
                    ))}
