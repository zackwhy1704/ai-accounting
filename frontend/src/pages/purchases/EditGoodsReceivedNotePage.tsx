...
import { useContacts, useBills, useGoodsReceivedNote, useUpdateGoodsReceivedNote, useGRNActivity, type InvoiceActivityEvent } from "../../lib/hooks"
...
export default function EditGoodsReceivedNotePage() {
  ...
  const { data: activity } = useGRNActivity(id)
    ...
    if (data && !populated.current) {
      ...
      if (data.line_items && data.line_items.length > 0) {
        setLineItems(data.line_items.map((li: any) => ({
          ...
          quantity_ordered: li.quantity_ordered || 0,
          quantity_received: li.quantity_received || 1,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast("Failed to update GRN", "warning")
    }
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        ...
        <div className="text-2xl font-semibold tracking-tight text-foreground">Edit Goods Received Note</div>
      </div>
      ...
      <Card className="rounded-2xl border-border bg-card p-6 shadow-sm">
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          ...
          <Button type="button" onClick={handleSave} disabled={updateGRN.isPending || !contactId || !receivedDate || !lineItems.some(li => li.description.trim())} className="h-9 rounded-xl bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] px-3 text-xs font-semibold text-white hover:opacity-95">
            {updateGRN.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Saving...</> : "Save Changes"}
          </Button>
        </div>
      </Card>
