...
export default function EditVendorCreditPage() {
  ...
  useEffect(() => {
    if (data && !populated.current) {
      ...
      setIssueDate(data.credit_date ? data.credit_date.slice(0, 10) : "")
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast("Failed to update vendor credit", "warning")
    }
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="rounded-2xl border-border bg-card p-6 shadow-sm">
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
          ...
          <div>
            <label className="mb-1.5 block text-xs font-medium text-muted-foreground">Related Bill</label>
        ...
        <div className="mt-6 flex items-center justify-end gap-2">
          ...
          <Button type="button" onClick={handleSave} disabled={updateVendorCredit.isPending} className="h-9 rounded-xl bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] px-3 text-xs font-semibold text-white hover:opacity-95">
