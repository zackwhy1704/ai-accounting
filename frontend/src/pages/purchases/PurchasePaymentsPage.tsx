                      ...
                      <TableRow key={p.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: "Void", icon: <XCircle className="h-3.5 w-3.5" />, onClick: () => { if (confirm("Void this payment? This reverses the GL entries and restores the bill balance. Cannot be undone.")) api.patch(`/purchase-payments/${p.id}/status`, null, { params: { status: "void" } }).then(() => { queryClient.invalidateQueries({ queryKey: ["purchase-payments"] }); queryClient.invalidateQueries({ queryKey: ["bills"] }); toast("Payment voided", "success") }).catch((e: any) => toast(e?.response?.data?.detail ?? "Failed to void payment", "warning")) }, danger: true, dividerBefore: true, disabled: p.status === "void" },
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if (confirm(`Delete payment ${p.payment_no}? Bill balance will be restored. This cannot be undone.`)) api.delete(`/purchase-payments/${p.id}`).then(() => { queryClient.invalidateQueries({ queryKey: ["purchase-payments"] }); queryClient.invalidateQueries({ queryKey: ["bills"] }); toast("Payment deleted", "success") }).catch((e: any) => toast(e?.response?.data?.detail ?? "Failed to delete payment", "warning")) }, danger: true },
                          ]} />
                        </TableCell>
                      </TableRow>
                    ))}
