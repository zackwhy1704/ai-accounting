                  ...
                  <TableRow key={p.id} className="border-border hover:bg-muted/50">
                    ...
                    <TableCell className="text-right">
                      <RowActionsMenu actions={[
                        ...
                        { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if (p.status !== "void" && p.status !== "draft") { alert("Please void this payment first before deleting."); return } if (confirm(`Delete payment ${p.payment_no ?? ""}? This cannot be undone.`)) api.delete(`/purchase-payments/${p.id}`).then(() => queryClient.invalidateQueries({ queryKey: ["purchase-payments"] })) }, danger: true, disabled: p.status !== "void" && p.status !== "draft" },
                      ]} />
                    </TableCell>
                  </TableRow>
                ))}
