                      ...
                      <TableRow key={dn.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: "Make Payment", icon: <CreditCard className="h-3.5 w-3.5" />, onClick: () => { const bal = (dn.total ?? 0) - ((dn as any).amount_paid ?? 0); navigate(`/purchases/payments/new?contact_id=${dn.contact_id}&amount=${bal.toFixed(2)}&debit_note_id=${dn.id}`) }, dividerBefore: true, disabled: dn.status === "void" || dn.status === "applied" || ((dn.total ?? 0) - ((dn as any).amount_paid ?? 0)) <= 0 },
                            { label: "Void", icon: <XCircle className="h-3.5 w-3.5" />, onClick: () => { if (dn.status === "applied") { toast("This debit note has a payment applied. Void the payment first before voiding the debit note.", "warning"); return } if (confirm("Void this debit note? This reverses the GL entries and cannot be undone.")) patch(dn.id, "void", "Debit note voided") }, danger: true, dividerBefore: true, disabled: dn.status === "void" },
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if (dn.status === "applied") { toast("This debit note has a payment applied. Void the payment first, then void the debit note before deleting.", "warning"); return } if (dn.status === "issued") { toast("Please void this debit note first before deleting.", "warning"); return } if (confirm(`Delete debit note ${dn.debit_note_number ?? ""}? This cannot be undone.`)) deleteDN.mutate(dn.id, { onSuccess: () => toast("Debit note deleted", "success"), onError: (e: any) => toast(e?.response?.data?.detail ?? "Failed to delete debit note", "warning") }) }, danger: true, disabled: dn.status === "applied" },
                          ]} />
                        </TableCell>
                      </TableRow>
                    ))}
