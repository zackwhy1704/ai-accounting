                      ...
                      <TableRow key={r.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if (r.status !== "void" && r.status !== "draft") { toast("Please void this refund first before deleting.", "warning"); return } if (confirm(`Delete refund ${r.refund_no}? This cannot be undone.`)) api.delete(`/purchase-refunds/${r.id}`).then(() => { queryClient.invalidateQueries({ queryKey: ["purchase-refunds"] }); toast("Refund deleted", "success") }).catch((e: any) => toast(e?.response?.data?.detail ?? "Failed to delete refund", "warning")) }, danger: true, disabled: r.status !== "void" && r.status !== "draft" },
                          ]} />
                        </TableCell>
                      </TableRow>
                    ))}
