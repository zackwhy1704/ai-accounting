...
interface SSTSalesDetailReport {
  items: SSTSalesItem[]
  total_taxable: number
  total_tax: number
}

export default function SSTSalesDetailPage() {
  ...
  const { data, isLoading, isFetching, isError, error } = useQuery<SSTSalesDetailReport>({
    queryKey: ["report-sst-sales-detail", queryParams],
    queryFn: () => api.get(`/reports/sst-sales-detail?start_date=${queryParams.fromDate}&end_date=${queryParams.toDate}`).then(r => r.data),
  })
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <div className="mt-1 text-2xl font-semibold tracking-tight text-foreground">SST Sales Detail</div>
        <div className="mt-1 text-sm text-muted-foreground">Sales tax detail by invoice line item</div>
      </div>
      {data && data.items.length > 0 && (
        <div className="flex gap-2 print:hidden">
          <Button variant="outline" size="sm" onClick={() => downloadCSV(`sst-sales-detail-${queryParams.fromDate}-${queryParams.toDate}.csv`, [
            ["SST Sales Detail", `${queryParams.fromDate} to ${queryParams.toDate}`],
            [],
            ["Invoice #", "Date", "Customer", "Description", "Qty", "Unit Price", "Taxable Amount", "Tax Rate %", "Tax Amount"],
            ...data.items.map(i => [i.invoice_number, i.date, i.customer_name, i.description, String(i.quantity), i.unit_price.toFixed(2), i.taxable_amount.toFixed(2), i.tax_rate.toFixed(2), i.tax_amount.toFixed(2)]),
            [],
      ...
      <Card className="rounded-2xl border-border bg-card shadow-[0_0_0_1px_rgba(15,23,42,0.06),0_18px_55px_rgba(2,6,23,0.08)] overflow-hidden">
        ...
        <Card className="rounded-2xl border-border bg-card p-4 shadow-sm"><QueryError error={error} message="Couldn't generate this report." /></Card>
          ...
          <div className="py-12 text-center">
            <div className="text-sm font-semibold text-foreground">No SST sales records found</div>
            ...
            <tbody>
              {data.items.map((item, i) => (
                <tr key={i} className="border-b border-border last:border-0 hover:bg-muted/30">
                  <td className="px-4 py-2.5 text-sm font-medium text-foreground">{item.invoice_number}</td>
                  <td className="px-4 py-2.5 text-sm text-foreground">{item.date}</td>
                  <td className="px-4 py-2.5 text-sm text-foreground">{item.customer_name}</td>
