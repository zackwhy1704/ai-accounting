...
export default function AgedPayablesPage() {
  ...
  const { data, isLoading, isFetching, isError, error } = useQuery<AgedPayablesReport>({
      ...
      {hasData && (
        <div className="flex gap-2 print:hidden">
          <Button variant="outline" size="sm" onClick={() => {
            const rows: string[][] = [
              ["Aged Payables", `As of ${data!.as_of_date}`],
              [],
              ["Bill", "Supplier", "Due Date", "Days Overdue", "Aging", "Amount Due"],
            ]
            BUCKET_ORDER.forEach(b => (data!.buckets[b] ?? []).forEach(e => {
              rows.push([e.bill_number, e.contact_name, e.due_date ?? "", String(e.days_overdue), BUCKET_LABELS[b], e.amount_due.toFixed(2)])
            }))
            rows.push([], ["", "", "", "", "Grand Total", data!.grand_total.toFixed(2)])
            downloadCSV(`aged-payables-${data!.as_of_date}.csv`, rows)
          }}>
