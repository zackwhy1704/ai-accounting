...
export default function InvoiceSummaryPage() {
  ...
  const { data, isLoading, isFetching, isError, error } = useQuery<InvoiceSummaryReport>({
    ...
    <div className="flex flex-col gap-4">
      ...
      {(data?.items?.length ?? 0) > 0 && (
        <div className="flex gap-2 print:hidden">
          <Button variant="outline" size="sm" onClick={() => downloadCSV(`invoice-summary-${data?.start_date}-${data?.end_date}.csv`, [
            ["Invoice Summary", `${data?.start_date} to ${data?.end_date}`],
            [],
            ["Customer", "Status", "Count", "Total", "Paid", "Balance"],
            ...(data?.items ?? []).map(i => [i.customer_name, i.status, String(i.count), i.total.toFixed(2), i.amount_paid.toFixed(2), i.balance.toFixed(2)]),
          ])}>
