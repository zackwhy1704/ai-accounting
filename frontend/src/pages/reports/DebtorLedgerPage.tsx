...
export default function DebtorLedgerPage() {
  ...
  const { data, isLoading, isFetching, isError, error } = useQuery<DebtorLedgerReport>({
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <div className="mt-1 text-sm text-muted-foreground">Running Debit/Credit ledger per customer</div>
      </div>
        ...
        <div className="flex gap-2 print:hidden">
          <Button variant="outline" size="sm" onClick={() => {
            const rows: string[][] = [
              ...
              [],
              ["Customer", "Date", "Reference", "Type", "Debit", "Credit", "Balance"],
            ]
            data?.customers?.forEach(c => {
              c.lines.forEach(l => {
                rows.push([c.customer_name, l.date ?? "", l.reference, l.type, l.debit > 0 ? l.debit.toFixed(2) : "", l.credit > 0 ? l.credit.toFixed(2) : "", l.balance.toFixed(2)])
              })
              rows.push(["", "", "", "Subtotal", c.total_debit.toFixed(2), c.total_credit.toFixed(2), c.balance.toFixed(2)])
            })
            rows.push([], ["", "", "", "Grand Total", (data?.grand_total_debit ?? 0).toFixed(2), (data?.grand_total_credit ?? 0).toFixed(2), (data?.grand_total_balance ?? 0).toFixed(2)])
              ...
              {(data?.customers ?? []).map((customer, ci) => (
                <Fragment key={ci}>
                  <tr className="bg-muted/20">
                    <td colSpan={6} className="px-4 py-2.5 text-sm font-semibold text-foreground">{customer.customer_name}</td>
                  </tr>
                  {customer.lines.map((l, li) => (
                    <tr key={`${ci}-${li}`} className="border-b border-border hover:bg-muted/30">
                  ...
                  <tr className="border-b border-border bg-muted/10">
                    ...
                    <td className="px-4 py-2 text-right text-sm tabular-nums font-semibold text-foreground">{formatCurrency(customer.total_debit)}</td>
                    <td className="px-4 py-2 text-right text-sm tabular-nums font-semibold text-foreground">{formatCurrency(customer.total_credit)}</td>
                    <td className="px-4 py-2 text-right text-sm tabular-nums font-semibold text-foreground">{formatCurrency(customer.balance)}</td>
                  </tr>
                </Fragment>
              ))}
