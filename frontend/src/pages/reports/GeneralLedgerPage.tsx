              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {acc.entries.map((entry, i) => (
                    <tr
                      key={i}
                      className={`border-b border-border last:border-0 hover:bg-muted/30 ${(entry as any).source_id ? "cursor-pointer" : ""}`}
                      title={(entry as any).source_id ? "Open source document" : undefined}
                      onClick={() => openDoc((entry as any).source, (entry as any).source_id)}
                    >
