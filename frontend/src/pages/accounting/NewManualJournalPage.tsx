...
import { useCreateManualJournal, useAccounts, useContacts } from "../../lib/hooks"
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 py-8">
      <div>
        ...
        <h1 className="mt-1 text-2xl font-bold text-foreground">New Journal Entry</h1>
        <p className="mt-1 text-sm text-muted-foreground">Record a manual general ledger entry</p>
      </div>
      ...
      <Card className="rounded-2xl border-border bg-card p-6 shadow-[0_0_0_1px_rgba(15,23,42,0.06),0_18px_55px_rgba(2,6,23,0.08)]">
        ...
        <div className="mt-6 flex justify-end gap-3">
          ...
          <Button
            onClick={handleSave}
            disabled={createJournal.isPending || !isBalanced || !hasLines || totalDebit === 0}
            className="bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] text-white"
          >
            {createJournal.isPending ? "Saving..." : "Save Journal Entry"}
          </Button>
        </div>
      </Card>
    </div>
