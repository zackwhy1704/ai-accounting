                      ...
                      <TableRow key={inv.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: t("invoices.void"), icon: <XCircle className="h-3.5 w-3.5" />, onClick: () => { if ((inv.amount_paid ?? 0) > 0) { alert("This invoice has payments applied. Please void or delete the payments first before voiding the invoice."); return } if (confirm("Void this invoice? This will reverse the GL entries.")) updateInvoiceStatus.mutate({ id: inv.id, status: "void" }) }, danger: true, dividerBefore: true, disabled: inv.status === "void" || inv.status === "cancelled" },
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if ((inv.amount_paid ?? 0) > 0) { alert("This invoice has payments applied. Please void the payments first, then void the invoice before deleting."); return } if (inv.status !== "draft" && inv.status !== "void" && inv.status !== "cancelled") { alert("Please void this invoice first before deleting. This reverses the GL entries."); return } if (confirm(`Delete invoice ${inv.invoice_number}? This cannot be undone.`)) deleteInvoice.mutate(inv.id) }, danger: true, disabled: inv.status === "paid" },
                          ]} />
                        </TableCell>
                      </TableRow>
                      )
