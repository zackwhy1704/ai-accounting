...
import { Plus, Download, Search, CalendarDays, SlidersHorizontal, Filter, MoreHorizontal } from "lucide-react"
...
export default function InvoicesPage() {
  ...
  const statusTabs = [
    ...
    { label: t("invoices.draft"), value: "draft" },
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button type="button" className="h-9 rounded-xl bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] px-3 text-xs font-semibold text-white shadow-[0_0_0_1px_rgba(124,157,255,0.25),0_16px_40px_rgba(0,0,0,0.35)] hover:opacity-95">
          ...
          <div className="mt-4">
            {isLoading ? (
              ...
              <div className="rounded-2xl border border-border bg-card px-6 py-10 text-center">
                ...
                <Button type="button" className="mt-6 h-9 rounded-xl bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] px-3 text-xs font-semibold text-white"><Plus className="mr-2 h-4 w-4" /> {t("invoices.create")}</Button>
              </div>
            ) : (
                    ...
                    {rows.map(inv => (
                      <TableRow key={inv.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground"><MoreHorizontal className="h-4 w-4" /></Button>
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
