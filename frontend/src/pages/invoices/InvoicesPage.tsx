                      ...
                      <TableRow key={inv.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: t("invoices.addPayment"), icon: <CreditCard className="h-3.5 w-3.5" />, onClick: () => navigate(`/sales/payments/new?invoice_id=${inv.id}&contact_id=${inv.contact_id}&amount=${balance.toFixed(2)}`), disabled: inv.status === "void" || inv.status === "draft" || inv.status === "paid" || balance <= 0 },
                            ...
                            { label: t("invoices.void"), icon: <XCircle className="h-3.5 w-3.5" />, onClick: () => { if ((inv.amount_paid ?? 0) > 0) { toast("This invoice has payments applied. Please void or delete the payments first before voiding the invoice.", "warning"); return } if (confirm("Void this invoice? This will reverse the GL entries.")) updateInvoiceStatus.mutate({ id: inv.id, status: "void" }, { onSuccess: () => toast("Invoice voided", "success"), onError: (e: any) => toast(e?.response?.data?.detail ?? "Failed to void invoice", "warning") }) }, danger: true, dividerBefore: true, disabled: inv.status === "void" || inv.status === "cancelled" },
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if ((inv.amount_paid ?? 0) > 0) { toast("This invoice has payments applied. Please void the payments first, then void the invoice before deleting.", "warning"); return } if (inv.status !== "draft" && inv.status !== "void" && inv.status !== "cancelled") { toast("Please void this invoice first before deleting. This reverses the GL entries.", "warning"); return } if (confirm(`Delete invoice ${inv.invoice_number}? This cannot be undone.`)) deleteInvoice.mutate(inv.id, { onSuccess: () => toast("Invoice deleted", "success"), onError: (e: any) => toast(e?.response?.data?.detail ?? "Failed to delete invoice", "warning") }) }, danger: true, disabled: inv.status === "paid" },
                          ]} />
                        </TableCell>
                      </TableRow>
                      )
