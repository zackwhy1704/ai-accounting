...
export default function EditBankTransactionPage() {
  const { id } = useParams()
  const navigate = useNavigate()
  const { toast } = useToast()
  const { data } = useBankTransaction(id)
  ...
  useEffect(() => {
    if (data && !populated.current) {
      ...
      setForm({
        transaction_date: data.transaction_date ? data.transaction_date.slice(0, 10) : new Date().toISOString().split("T")[0],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate(
      ...
      },
      {
        ...
        onError: () => toast("Failed to update transaction", "warning"),
      }
    )
  }
  ...
  return (
    <div className="flex flex-col gap-4">
      <div>
        ...
        <div className="mt-1 text-2xl font-semibold tracking-tight text-foreground">Edit Transaction</div>
        <div className="mt-1 text-sm text-muted-foreground">Update this bank transaction</div>
      </div>
      ...
      <Card className="rounded-2xl border-border bg-card p-6 shadow-[0_0_0_1px_rgba(15,23,42,0.06),0_18px_55px_rgba(2,6,23,0.08)]">
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button
              type="submit"
              disabled={updateMutation.isPending || !form.transaction_date || !form.description || !form.amount}
              className="h-9 rounded-xl bg-gradient-to-r from-[#7C9DFF] to-[#4D63FF] px-4 text-xs font-semibold text-white"
            >
              {updateMutation.isPending ? "Saving..." : "Save Transaction"}
            </Button>
          </div>
        </form>
      </Card>
