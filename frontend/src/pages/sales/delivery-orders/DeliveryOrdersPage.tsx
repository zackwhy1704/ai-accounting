...
import { useDeliveryOrdersPage, useContacts, useUpdateDeliveryOrderStatus, useDeleteDeliveryOrder, useDebounce } from "../../../lib/hooks"
...
export default function DeliveryOrdersPage() {
  ...
  const { data: deliveryOrdersPage, isLoading } = useDeliveryOrdersPage({ status: tab === "all" ? undefined : tab, search: debouncedSearch, page, limit: 50 })
  const deliveryOrders = deliveryOrdersPage?.items ?? []
  ...
  const rows = useMemo(() => {
    ...
    if (dateFrom) filtered = filtered.filter(i => (i.delivery_date || "") >= dateFrom)
    if (dateTo) filtered = filtered.filter(i => (i.delivery_date || "") <= dateTo)
    return filtered
  }, [deliveryOrders, contactMap, contactFilter, dateFrom, dateTo])
          ...
          <div className="mt-4">
            {isLoading ? (
              ...
              <div className="overflow-hidden rounded-2xl border border-border bg-card">
                ...
                <PaginationControls page={page} pages={deliveryOrdersPage?.pages ?? 1} total={deliveryOrdersPage?.total ?? 0} limit={50} onPageChange={setPage} />
              </div>
            )}
          </div>
        </Tabs>
