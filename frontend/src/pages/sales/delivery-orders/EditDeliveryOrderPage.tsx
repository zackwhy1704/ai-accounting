...
import { useDeliveryOrder, useUpdateDeliveryOrder, useContacts, useTaxRates, useDeliveryOrderActivity, type InvoiceActivityEvent } from "../../../lib/hooks"
...
export default function EditDeliveryOrderPage() {
  ...
  const { data: activity } = useDeliveryOrderActivity(id)
  ...
  const [lineItems, setLineItems] = useState<LineItem[]>([
    { description: "", quantity: 1, unit_price: 0, discount: 0, discount_mode: "percent", amount: 0, tax_code_id: "", tax_rate: 0 },
  ])
  ...
  const updateLineItem = (index: number, field: keyof LineItem, value: string | number) => {
    setLineItems(prev => {
      ...
      updated[index].amount = item.quantity * item.unit_price - lineDiscountAmount(item)
  ...
  const addLineItem = () => {
    setLineItems(prev => [...prev, { description: "", quantity: 1, unit_price: 0, discount: 0, discount_mode: "percent", amount: 0, tax_code_id: "", tax_rate: 0 }])
  }
    ...
    try {
      await updateDeliveryOrder.mutateAsync({
        ...
        line_items: lineItems.map(li => ({
          ...
          discount: lineDiscountAmount(li),
