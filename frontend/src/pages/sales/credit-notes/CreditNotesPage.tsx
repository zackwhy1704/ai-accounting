                      ...
                      <TableRow key={row.id} className="border-border hover:bg-muted/50">
                        ...
                        <TableCell className="text-right">
                          <RowActionsMenu actions={[
                            ...
                            { label: "Delete", icon: <Trash2 className="h-3.5 w-3.5" />, onClick: () => { if ((row.credit_applied ?? 0) > 0) { alert("This credit note has applied amounts. Please use \"Remove Applications\" first to restore invoice balances before deleting."); return } if (confirm(`Delete credit note ${row.credit_note_number ?? ""}? This cannot be undone.`)) deleteCreditNote.mutate(row.id) }, danger: true, disabled: row.status === "applied" },
                          ]} />
                        </TableCell>
                      </TableRow>
                    ))}
